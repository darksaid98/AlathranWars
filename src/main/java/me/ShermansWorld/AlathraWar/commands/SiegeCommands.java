...
public class SiegeCommands implements CommandExecutor {
    public SiegeCommands(final Main plugin) {
        plugin.getCommand("siege").setExecutor(this);
    }
...
//        if (!(sender instanceof Player)) return;
    ...
    private static void siegeStop(CommandSender sender, String[] args) {
        if (!sender.hasPermission("AlathraWar.admin")) {
            sender.sendMessage(Helper.chatLabel() + Helper.color("&cYou do not have permission to do this"));
            return;
        }
        HashSet<Siege> sieges = SiegeData.getSieges();
        if (sieges.isEmpty()) {
            sender.sendMessage(Helper.chatLabel() + "There are currently no sieges in progress");
            return;
        }
        ...
        War war = WarData.getWar(args[1]);
        if (war == null) {
            sender.sendMessage(Helper.chatLabel() + "That war does not exist! /siege stop [war] [town]");
            return;
        }

        for (Siege siege : sieges) {
            if (siege.getTown().getName().equalsIgnoreCase(args[2])) {
                sender.sendMessage(Helper.chatLabel() + "siege cancelled");
                Bukkit.broadcastMessage(Helper.chatLabel() + "The siege at " + siege.getTown().getName() + " has been cancelled by an admin");
                //siege.clearBeacon();
                siege.stop();
                return;
            }
        }
        sender.sendMessage(Helper.chatLabel() + "A siege could not be found with this town name! Type /siege list to view current sieges");
    }
    ...
    private static void siegeAbandon(CommandSender sender, String[] args) {
        ...
        War war = WarData.getWar(args[1]);
        if (war == null) {
            p.sendMessage(Helper.chatLabel() + "That war does not exist! /siege abandon [war] [town]");
            return;
        }
        ...
        Bukkit.broadcastMessage(Helper.chatLabel() + "The siege at " + siege.getTown().getName() + " has been abandoned.");
    ...
    private static void siegeList(CommandSender sender, String[] args) {
        ...
        sender.sendMessage(Helper.chatLabel() + "Sieges currently in progress:");
        for (Siege siege : sieges) {
    ...
    public boolean onCommand(final CommandSender sender, final Command cmd, final String label, final String[] args) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Helper.chatLabel() + "Invalid Arguments. /siege help");
            return true;
        }
