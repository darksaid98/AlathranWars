...
public class RaidCommands implements CommandExecutor {
    ...
    protected static void joinRaid(CommandSender p, String[] args, boolean admin) {
        ...
        if (raid != null) {
            if (raid.getWar().getName().equalsIgnoreCase(args[1])) {
                ...
                if (joiner == null) {
                    p.sendMessage(Helper.chatLabel() + "Player not found! Is something broken?");
                    return;
                }
    ...
    protected static void leaveRaid(CommandSender player, String[] args, boolean admin) {
        ...
        if (admin) {
            ...
            if (p == null) {
                player.sendMessage(Helper.chatLabel() + "Error: player not found!");
                return;
            }
        }
        if (p == null) {
        ...
        if (raid != null) {
            ...
            p.sendMessage(Helper.chatLabel() + "No raid is gathering in this town or this town does not exist!");
            if (admin)
                player.sendMessage(Helper.chatLabel() + "No raid is gathering in this town or this town does not exist!");
        }
    }
    ...
    private static void abandonRaid(CommandSender p, String[] args) {
            ...
            if (raid.getWar().getName().equalsIgnoreCase(args[1])) {
                ...
                if (raid.getOwner().getName().equalsIgnoreCase(p.getName())) {
                    ...
                    if (raid.getPhase() == RaidPhase.GATHER || raid.getPhase() == RaidPhase.START || raid.getPhase() == RaidPhase.TRAVEL || raid.getPhase() == RaidPhase.COMBAT) {
                        ...
                        Bukkit.broadcastMessage(Helper.chatLabel() + "The raid on " + raid.getRaidedTown().getName() + " has been abandoned by " + raid.getRaiderSide());
                        ...
                        p.sendMessage(Helper.chatLabel() + "You cannot abandon the raid on " + raid.getRaidedTown().getName() + "! It has a problem!");
                    }
                } else {
                    fail(p, args, "owner");
                }
        ...
        p.sendMessage(Helper.chatLabel() + "This raid does not exist!");
    }

    private static void listRaids(CommandSender p, String[] args) {
        if (RaidData.getRaids().isEmpty()) {
            p.sendMessage(Helper.chatLabel() + "There are currently no Raids in progress");
            return;
        }
        p.sendMessage(Helper.chatLabel() + "Raids currently in progress:");
    ...
    private static void fail(CommandSender p, String[] args, String type) {
        switch (type) {
            case "permissions" ->
                    p.sendMessage(Helper.chatLabel() + "&cYou do not have permission to do this");
            case "syntax" -> p.sendMessage(Helper.chatLabel() + "Invalid Syntax. /raid help");
            case "noRaids" ->
                    p.sendMessage(Helper.chatLabel() + "There are currently no Raids in progress");
            case "badName" ->
                    p.sendMessage(Helper.chatLabel() + "A raid could not be found with this Name! Type /raid list to view current raids");
            case "owner" ->
                    p.sendMessage(Helper.chatLabel() + "Only the player who started the raid can abandon it.");
            default -> p.sendMessage(Helper.chatLabel() + "Invalid Arguments. /raid help");
        }
    }
