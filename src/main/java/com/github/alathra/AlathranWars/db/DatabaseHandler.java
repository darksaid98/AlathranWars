...
public class DatabaseHandler implements Reloadable {
     ...
     */
    public DatabaseHandler(AlathranWars alathranWars) {
    ...
    private void openConnection() {
        ...
        switch (db) {
            case HSQLDB, H2 -> {
                ...
                hikariConfig.addDataSourceProperty("url", "jdbc:%s:%s".formatted(
                    db.getJdbcPrefix(),
                    alathranWars.getDataFolder().getAbsolutePath() + File.separatorChar + subfolder + File.separatorChar + (db.equals(DatabaseType.H2) ? fileName : fileNameWithExtension) + connectionProperties
                ));
            }
            case MYSQL, MARIADB -> {
                hikariConfig.setDataSourceClassName(db.getDataSourceClassName());
        ...
        hikariConfig.setPoolName("%s-hikari".formatted(AlathranWars.getInstance().getName()));
