...
public class Siege
{
    ...
    public Siege(final int id, final War war, final Town town, final String attackers, final String defenders, final boolean side1AreAttackers, final boolean side2AreAttackers) {
        this.siegeTicks = 0;
    ...
    public void start() {
        ...
        this.side1AreAttackers = Main.data2.getConfig().getBoolean("Sieges." + String.valueOf(this.id) + ".side1areattackers");
        this.side2AreAttackers = Main.data2.getConfig().getBoolean("Sieges." + String.valueOf(this.id) + ".side2areattackers");
        ...
        this.bukkitId[0] = Bukkit.getServer().getScheduler().scheduleSyncRepeatingTask((Plugin)Main.getInstance(), (Runnable)new Runnable() {
            ...
            public void run() {
                ...
                if (Siege.this.siegeTicks >= Siege.this.MAXSIEGETICKS) {
                    ...
                    Main.data2.getConfig().set("Sieges." + String.valueOf(Siege.this.id) + ".siegeticks", (Object)Siege.this.siegeTicks);
                    Main.data2.saveConfig();
                    for (final String playerName : Siege.this.attackerPlayers) {
                        try {
                            if (WorldCoord.parseWorldCoord(Main.getInstance().getServer().getPlayer(playerName)).getTownBlock().isHomeBlock() && WorldCoord.parseWorldCoord(Main.getInstance().getServer().getPlayer(playerName)).getTownBlock().getTown().equals(town) && !Bukkit.getPlayer(playerName).isDead()) {
                    ...
                    if (attackersAreOnHomeBlock && defendersAreOnHomeBlock) {
                        if (this.homeBlockControl != 1) {
                            for (final String playerName : Siege.this.attackerPlayers) {
                                try {
                                    Bukkit.getPlayer(playerName).sendMessage(String.valueOf(Helper.Chatlabel()) + "HomeBlock at " + Siege.this.town.getName() + " contested!");
                                }
                        ...
                        if (this.homeBlockControl != 2) {
                            for (final String playerName : Siege.this.attackerPlayers) {
                                try {
                                    Bukkit.getPlayer(playerName).sendMessage(String.valueOf(Helper.Chatlabel()) + "Attackers have captured the HomeBlock at " + Siege.this.town.getName() + "! +1 Attacker Points per second");
                                }
                        ...
                        Main.data2.getConfig().set("Sieges." + String.valueOf(Siege.this.id) + ".attackerpoints", (Object)Siege.this.attackerPoints);
                        Main.data2.saveConfig();
                    }
                    else {
                        if (this.homeBlockControl != 3) {
                            for (final String playerName : Siege.this.attackerPlayers) {
                                try {
                                    Bukkit.getPlayer(playerName).sendMessage(String.valueOf(Helper.Chatlabel()) + "Defenders retain control of the HomeBlock at " + Siege.this.town.getName() + "! +1 Defender Points per second");
                                }
                        ...
                        Main.data2.getConfig().set("Sieges." + String.valueOf(Siege.this.id) + ".defenderpoints", (Object)Siege.this.defenderPoints);
                        Main.data2.saveConfig();
                        this.homeBlockControl = 3;
                    }
                    if (Siege.this.siegeTicks % 6000 == 0) {
                        Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + "Report on the siege of " + Siege.this.town.getName() + ":");
                        Bukkit.broadcastMessage("Attacker Points - " + String.valueOf(Siege.this.attackerPoints));
                        Bukkit.broadcastMessage("Defender Points - " + String.valueOf(Siege.this.defenderPoints));
                    }
                }
            }
        }, 0L, 200L);
    ...
    public void attackersWin(final Player owner) {
        ...
        Nation nation = null;
        Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + "The attackers from " + this.attackers + " have won the siege of " + this.town.getName() + "!");
        try {
            ...
            for (final String playerName : this.attackerPlayers) {
                try {
                    Bukkit.getPlayer(playerName).sendMessage(String.valueOf(Helper.Chatlabel()) + "The town of " + this.town.getName() + " could not be occupied because the player who started the siege is not part of a nation");
                }
        ...
        if (nation != null) {
            ...
            Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + "The town of " + this.town.getName() + " has been placed under occupation by " + nation.getName() + "!");
        }
        final OfflinePlayer offlinePlayer = Bukkit.getOfflinePlayer(owner.getName());
        ...
        if (this.town.getAccount().getHoldingBalance() > 80000.0) {
            ...
            if (this.town.getAccount().getHoldingBalance() < 20000.0) {
                ...
                Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + "The town of " + this.town.getName() + " has been destroyed by " + this.getAttackers() + "!");
        ...
        Bukkit.broadcastMessage("The town of " + this.town.getName() + " has been sacked by " + this.getAttackers() + ", valuing $" + String.valueOf(amt));
        Main.econ.depositPlayer(offlinePlayer, amt);
    ...
    public void defendersWin() {
        ...
        Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + "The defenders from " + this.defenders + " have won the siege of " + this.town.getName() + "!");
        Bukkit.broadcastMessage(String.valueOf(Helper.Chatlabel()) + this.town.getName() + " has recovered the attackers' war chest, valued at $20,000");
    ...
    public void createBeacon() {
        try {
            ...
            int homeBlockZ = this.town.getHomeBlock().getZ() * 16;
            if (homeBlockX > 0) {
                homeBlockX -= 8;
            }
            if (homeBlockZ > 0) {
            ...
            int homeBlockY = 253;
            final Location beaconLoc = new Location(this.town.getWorld(), (double)homeBlockX, (double)(homeBlockY - 2), (double)homeBlockZ);
            ...
            this.beaconLocs.add(new Location(world, (double)homeBlockX, (double)(homeBlockY - 1), (double)homeBlockZ));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX + 1), (double)(homeBlockY - 1), (double)homeBlockZ));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX - 1), (double)(homeBlockY - 1), (double)homeBlockZ));
            this.beaconLocs.add(new Location(world, (double)homeBlockX, (double)(homeBlockY - 1), (double)(homeBlockZ + 1)));
            this.beaconLocs.add(new Location(world, (double)homeBlockX, (double)(homeBlockY - 1), (double)(homeBlockZ - 1)));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX + 1), (double)(homeBlockY - 1), (double)(homeBlockZ + 1)));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX + 1), (double)(homeBlockY - 1), (double)(homeBlockZ - 1)));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX - 1), (double)(homeBlockY - 1), (double)(homeBlockZ + 1)));
            this.beaconLocs.add(new Location(world, (double)(homeBlockX - 1), (double)(homeBlockY - 1), (double)(homeBlockZ - 1)));
